...
def test_record_open_trade(app):
    ...
    with app.app_context():
        t = trade_db.record_open_trade(data, payload)
        assert t is not None
        assert t.symbol == 'AAPL'
        assert t.status == 'open'

        t2 = trade_db.record_open_trade(data, payload)
        assert t2 is None
...
def test_record_closed_trade(app, monkeypatch):
    ...
    with app.app_context():
        trade_db.record_open_trade(open_data, open_payload)
        ...
        monkeypatch.setattr(trade_db, 'api', SimpleNamespace(get_order=mock_get_order))
        monkeypatch.setattr(trade_db.time, 'sleep', lambda s: None)

        close_data = {'close_order_id': 'close123'}
        t = trade_db.record_closed_trade(close_data, {'symbol': 'AAPL', 'action': 'sell'}, position_obj)
